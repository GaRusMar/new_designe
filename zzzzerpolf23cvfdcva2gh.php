...
if ($host == "studiya-dizaynov.ru") {
	...
	///////////////////////////////////////////////////////////////////////
		...
		$email_body = $email_body. "<tr><td>Вопрос</td><td>".$textar."</td></tr>";
		...
		$email_body = $email_body. "<tr><td>Точка входа</td><td>".$url."</td></tr>";		
		$email_body = $email_body. "<tr><td>Название дома из СИП-панелей</td><td>".$titleCard."</td></tr></table>";


		$mail->From = 'studiya-dizaynov.ru <studia.remontov.zayavki@gmail.com>';
		$mail->FromName = 'studiya-dizaynov.ru,';
		$mail->AddAddress('studia.remontov.zayavki@gmail.com', 'ANALYTICS' );
		$mail->AddAddress('studia.remontov.zayavki@yandex.ru', 'ANALYTICS' );  

		//$mail->AddAddress('ulyamorozova797@gmail.com', 'ANALYTICS' );
		
		$mail->Subject = 'Новая заявка со Студии Дизайнов!';
		...
		if(!isset($hasError)) {
			if(!$mail->send()) {
				...
				header('Location: https://studiya-dizaynov.ru/success.php'); 			
			}
		}
	} else {
		echo 'Приносим извинения, но обращение к файлу напрямую запрещена.';
	}
