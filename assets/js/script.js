...
function init() {
    ...
    form.find('.empty-field').on('change', function(){
      ...
      $('#upload-file').on('change', function() {
        ...
        if(numb > 2){
            $(this).addClass('error');
            $(this).parent().find('.upload-text').text('Размер файла не должен превышать 2 МБ').css('color', '#f33e3e');
        } else {
            $(this).removeClass('error');
             $(this).parent().find('.upload-text').css('color', '#7b7b7b');        
        }
    });   
      ...
      btn.click(function(){
        if($(this).hasClass('disabled')) {
            ...
            return false;
        } else if ($(this).parent().parent().find('#upload-file').hasClass('error')) {
            return false;
        } else {
